...
$ApiBase = "https://dev.azure.com/$System_CollectionId/$System_TeamProjectId"

# Get latest coverage from master.
$masterBuildJson = (Invoke-WebRequest "$ApiBase/_apis/build/builds?branchName=refs/heads/master&api-version=5.0-preview.1").Content | ConvertFrom-Json
$masterLatestId = $masterBuildJson.value[0].id
$masterCoverageJson = (Invoke-WebRequest "$ApiBase/_apis/test/codecoverage?buildId=$masterLatestId&api-version=5.0-preview.1").Content | ConvertFrom-Json
...
$branchCoverageJson = (Invoke-WebRequest "$ApiBase/_apis/test/codecoverage?buildId=$Build_BuildId&api-version=5.0-preview.1").Content | ConvertFrom-Json
