...
class MailchimpApi:
    ...
    def get_merge_fields_for_list(self, list_id):
        endpoint = "lists/{0}/merge-fields?fields=merge_fields.tag,merge_fields.merge_id,merge_fields.name".format(  # noqa
            list_id
        )
        json_response = self._get(endpoint)
        return json_response
    ...
    def get_interests_for_interest_category(self, list_id, interest_category_id):
        endpoint = "lists/{0}/interest-categories/{1}/interests?fields=interests.id,interests.name".format(  # noqa
