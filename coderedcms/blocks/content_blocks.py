...
class CardBlock(BaseBlock):
    ...
    image = ImageChooserBlock(required=False, max_length=255, label=_("Image"))
    title = blocks.CharBlock(required=False, max_length=255, label=_("Title"))
    subtitle = blocks.CharBlock(required=False, max_length=255, label=_("Subtitle"))
    description = blocks.RichTextBlock(
        features=["bold", "italic", "ol", "ul", "hr", "link", "document-link"], label=_("Body")
    )
    links = blocks.StreamBlock(
        [("Links", ButtonBlock())], blank=True, required=False, label=_("Links")
    )
...
class ImageGalleryBlock(BaseBlock):
    ...
    collection = CollectionChooserBlock(required=True, label=_("Image Collection"))
...
class ModalBlock(ButtonMixin, BaseLayoutBlock):
    ...
    header = blocks.CharBlock(required=False, max_length=255, label=_("Modal heading"))
    content = blocks.StreamBlock([], label=_("Modal content"))
    footer = blocks.StreamBlock(
        [
            (
                "text",
                blocks.CharBlock(icon="fa-file-text-o", max_length=255, label=_("Simple Text")),
            ),
            ("button", ButtonBlock()),
        ],
        required=False,
...
class NavBaseLinkBlock(BaseBlock):
    display_text = blocks.CharBlock(required=False, max_length=255, label=_("Display text"))
...
class NavExternalLinkBlock(NavBaseLinkBlock):
    ...
    link = blocks.CharBlock(required=False, label=_("URL"))
...
class PriceListItemBlock(BaseBlock):
    ...
    image = ImageChooserBlock(required=False, label=_("Image"))
    name = blocks.CharBlock(requred=True, max_length=255, label=_("Name"))
    description = blocks.TextBlock(required=False, rows=4, label=_("Description"))
...
class PriceListBlock(BaseBlock):
    ...
    heading = blocks.CharBlock(required=False, max_length=255, label=_("Heading"))
    items = blocks.StreamBlock([("item", PriceListItemBlock())], label=_("Items"))
