...
class DownloadBlock(ButtonMixin, BaseBlock):
    ...
    automatic_download = blocks.BooleanBlock(required=False, label=_("Auto download"))
    downloadable_file = DocumentChooserBlock(required=False, label=_("Document link"))
...
class PageListBlock(BaseBlock):
    ...
    show_preview = blocks.BooleanBlock(required=False, default=False, label=_("Show body preview"))
    num_posts = blocks.IntegerBlock(default=3, label=_("Number of pages to show"))
        ...
        if hasattr(indexer, "get_index_children"):
            ...
            if value["classified_by"]:
                try:
                    ...
                    logger.warning(
                        "Tried to filter by ClassifierTerm in PageListBlock, but <%s.%s ('%s')>.get_index_children() did not return a queryset or is not a queryset of CoderedPage models.",  # noqa
...
class QuoteBlock(BaseBlock):
    ...
    text = blocks.TextBlock(required=True, rows=4, label=_("Quote Text"))
    author = blocks.CharBlock(required=False, max_length=255, label=_("Author"))
