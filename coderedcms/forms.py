...
FORM_FIELD_CHOICES = (
    ...
    (
        _("Date & Time"),
        (("date", _("Date")), ("time", _("Time")), ("datetime", _("Date and time"))),
    ),
    (_("File Upload"), (("file", _("Secure File - login required to access uploaded files")),)),
    (_("Other"), (("hidden", _("Hidden field")),)),
)
...
class SecureFileField(forms.FileField):
    ...
    def _check_whitelist(self, value):
        if cr_settings["PROTECTED_MEDIA_UPLOAD_WHITELIST"]:
            if (
                ...
                not in cr_settings["PROTECTED_MEDIA_UPLOAD_WHITELIST"]
            ):
    ...
    def _check_blacklist(self, value):
        if cr_settings["PROTECTED_MEDIA_UPLOAD_BLACKLIST"]:
            if (
                os.path.splitext(value.name)[1].lower()
                in cr_settings["PROTECTED_MEDIA_UPLOAD_BLACKLIST"]
            ):
...
class CoderedFormField(AbstractFormField):
    ...
    field_type = models.CharField(
        verbose_name=_("field type"), max_length=16, choices=FORM_FIELD_CHOICES, blank=True
    )


class SearchForm(forms.Form):
    s = forms.CharField(max_length=255, required=False, label=_("Search"))
    t = forms.CharField(
        widget=forms.HiddenInput, max_length=255, required=False, label=_("Page type")
    )
...
def get_page_model_choices():
    ...
    return (
        (page.__name__, re.sub(r"((?<=[a-z])[A-Z]|(?<!\A)[A-Z](?=[a-z]))", r" \1", page.__name__))
