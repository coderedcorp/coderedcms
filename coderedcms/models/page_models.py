...
class CoderedPage(WagtailCacheMixin, Page, metaclass=CoderedPageMeta):
    ...
    index_num_per_page = models.PositiveIntegerField(default=10, verbose_name=_("Number per page"))
    ...
    struct_org_actions = StreamField(
        [("actions", StructuredDataActionBlock())], blank=True, verbose_name=_("Actions")
    )
    ...
    content_walls = StreamField(
        [("content_wall", ContentWallBlock())], blank=True, verbose_name=_("Content Walls")
    )
    ...
    content_panels = Page.content_panels + [ImageChooserPanel("cover_image")]
        ...
        MultiFieldPanel(
            [
                HelpPanel(
                    ...
                    content=_(
                        "The fields below help define brand, contact, and storefront information to search engines. This information should be filled out on the site’s root page (Home Page). If your organization has multiple locations, then also fill this info out on each location page using that particular location’s info."  # noqa
                    ),
                ),
    ...
    settings_panels = Page.settings_panels + [StreamFieldPanel("content_walls")]
    ...
    def __init__(self, *args, **kwargs):
        ...
        template_choices = cr_settings["FRONTEND_TEMPLATES_PAGES"].get("*", ()) + cr_settings[
    ...
    def get_edit_handler(cls):
        ...
        if cls.integration_panels:
            panels.append(
                ObjectList(cls.integration_panels, heading="Integrations", classname="integrations")
            )
...
class CoderedArticlePage(CoderedWebPage):
    ...
    date_display = models.DateField(null=True, blank=True, verbose_name=_("Display publish date"))
    ...
    content_panels = CoderedWebPage.content_panels + [
        FieldPanel("caption"),
        MultiFieldPanel(
            [FieldPanel("author"), FieldPanel("author_display"), FieldPanel("date_display")],
            _("Publication Info"),
        ),
    ]
...
class CoderedArticleIndexPage(CoderedWebPage):
    ...
    show_images = models.BooleanField(default=True, verbose_name=_("Show images"))
    show_captions = models.BooleanField(default=True)
    show_meta = models.BooleanField(default=True, verbose_name=_("Show author and date info"))
    show_preview_text = models.BooleanField(default=True, verbose_name=_("Show preview text"))
...
class CoderedEventPage(CoderedWebPage, BaseEvent):
    ...
    calendar_color = ColorField(
        blank=True, help_text=_("The color that the event will use when displayed on a calendar.")
    )
    address = models.TextField(blank=True, verbose_name=_("Address"))
    content_panels = CoderedWebPage.content_panels + [
        MultiFieldPanel(
            [FieldPanel("calendar_color"), FieldPanel("address")], heading=_("Event information")
        ),
        InlinePanel("occurrences", min_num=1, heading=_("Dates and times")),
    ]
    ...
    def query_occurrences(self, num_of_instances_to_return=None, **kwargs):
        ...
        """
        event_instances = []
        occurrence_kwargs = {"from_date": kwargs.get("from_date", timezone.now().date())}
...
class CoderedEventIndexPage(CoderedWebPage):
    ...
    layout_panels = CoderedWebPage.layout_panels + [FieldPanel("default_calendar_view")]
...
class CoderedFormMixin(models.Model):
    ...
    button_text = models.CharField(
        max_length=255, default=_("Submit"), verbose_name=_("Button text")
    )
    ...
    def send_summary_mail(self, request, form, processed_data):
        ...
        for key, value in self.data_to_dict(processed_data, request).items():
            content.append("{0}: {1}".format(key.replace("_", " ").title(), value))

        content = "\n-------------------- \n".join(content)

        # Build email message parameters
        message_args = {"body": content, "to": addresses}
        if self.subject:
    ...
    preview_modes = [("form", _("Form")), ("landing", _("Thank you page"))]
        ...
        if request.method == "POST":
            ...
            if form.is_valid():
                ...
                form_submission = self.get_submission_class()(
                    form_data=json.dumps(processed_data, cls=self.encoder), page=self
                )
...
class CoderedSessionFormSubmission(SessionFormSubmission):
    ...
    def create_normal_submission(self, delete_self=True):
        ...
        submission = FormSubmission.objects.create(
            form_data=json.dumps(submission_data, cls=StreamFormJSONEncoder), page=self.page
        )
...
def create_submission_changed_revision(sender, **kwargs):
    ...
    CoderedSubmissionRevision.create_from_submission(
        submission,
        (CoderedSubmissionRevision.CREATED if created else CoderedSubmissionRevision.CHANGED),
    )
...
def create_submission_deleted_revision(sender, **kwargs):
    if not issubclass(sender, CoderedSessionFormSubmission):
        return
    submission = kwargs["instance"]
    CoderedSubmissionRevision.create_from_submission(submission, SubmissionRevision.DELETED)
...
class CoderedSteps(Steps):
    def __init__(self, page, request=None):
        ...
        has_steps = any(
            isinstance(struct_child.block, FormStepBlock) for struct_child in self.form_fields
        )
        if has_steps:
            steps = [
                CoderedStep(self, i, form_field) for i, form_field in enumerate(self.form_fields)
            ]
        else:
...
class CoderedStreamFormMixin(StreamFormMixin):
    ...
    def get_submission(self, request):
        ...
        if request.user.is_authenticated:
            user_submission = (
                Submission.objects.filter(user=request.user, page=self).order_by("-pk").first()
            )
        ...
        user_submission = (
            Submission.objects.filter(session_key=request.session.session_key, page=self)
            .order_by("-pk")
            .first()
        )
        if user_submission is None:
            return Submission(session_key=request.session.session_key, page=self, form_data="[]")
        return user_submission
...
class CoderedLocationPage(CoderedWebPage):
    ...
    phone_number = models.CharField(blank=True, max_length=255, verbose_name=_("Phone Number"))
    ...
    layout_panels = CoderedWebPage.layout_panels + [
        MultiFieldPanel(
            [FieldPanel("map_title"), FieldPanel("map_description")], heading=_("Map Layout")
        )
    ]

    settings_panels = CoderedWebPage.settings_panels + [
        MultiFieldPanel(
            [FieldPanel("auto_update_latlng"), FieldPanel("latitude"), FieldPanel("longitude")],
    ...
    def render_pin_description(self):
        return render_to_string("coderedcms/includes/map_pin_description.html", {"page": self})

    @property
    def render_list_description(self):
        return render_to_string("coderedcms/includes/map_list_description.html", {"page": self})
...
class CoderedLocationIndexPage(CoderedWebPage):
    ...
    layout_panels = CoderedWebPage.layout_panels + [
        MultiFieldPanel(
            [FieldPanel("center_latitude"), FieldPanel("center_longitude"), FieldPanel("zoom")],
    ...
    def geojson_data(self, viewport=None):
        ...
        return {"type": "FeatureCollection", "features": [location.to_geojson() for location in qs]}
