...
class CoderedPage(WagtailCacheMixin, Page, metaclass=CoderedPageMeta):
    ...
    index_num_per_page = models.PositiveIntegerField(
        default=10, verbose_name=_("Number per page")
    )
...
class CoderedArticlePage(CoderedWebPage):
    ...
    date_display = models.DateField(
        null=True, blank=True, verbose_name=_("Display publish date")
    )
...
class CoderedArticleIndexPage(CoderedWebPage):
    ...
    show_meta = models.BooleanField(
        default=True, verbose_name=_("Show author and date info")
    )
    show_preview_text = models.BooleanField(
        default=True, verbose_name=_("Show preview text")
    )
...
class CoderedEventPage(CoderedWebPage, BaseEvent):
    ...
    content_panels = CoderedWebPage.content_panels + [
        MultiFieldPanel(
            [FieldPanel("calendar_color"), FieldPanel("address")],
        ...
        for event in qs:
            ...
            for occurrence in occurrences:
                event_data = {
                    ...
                    "end": occurrence[1].strftime("%Y-%m-%dT%H:%M:%S")
                    if occurrence[1]
                    else "",
                    "description": "",
                }
        ...
        for key, val in form.cleaned_data.items():
            ...
            if type(val) == InMemoryUploadedFile or type(val) == TemporaryUploadedFile:
                ...
                processed_data[key] = "{0}{1}".format(
                    cr_settings["PROTECTED_MEDIA_URL"], path
                )
            else:
    ...
    def serve(self, request, *args, **kwargs):
        ...
        if request.method == "POST" and form.is_valid():
            ...
            if is_complete:
                ...
                return self.render_landing_page(
                    request, normal_submission, *args, **kwargs
                )
...
class CoderedLocationPage(CoderedWebPage):
    ...
    layout_panels = CoderedWebPage.layout_panels + [
        MultiFieldPanel(
            [FieldPanel("map_title"), FieldPanel("map_description")],
