...
class FormFieldBlock(StructBlock):
    ...
    def get_field(self, struct_value):
        return self.get_field_class(struct_value)(**self.get_field_kwargs(struct_value))
...
CHARFIELD_FORMATS = [("email", _("Email")), ("url", _("URL"))]
try:
...
class CheckboxesFieldBlock(OptionalFormFieldBlock):
    ...
    def get_field_kwargs(self, struct_value):
        kwargs = super(CheckboxesFieldBlock, self).get_field_kwargs(struct_value)
...
class DatePickerInput(forms.DateInput):
    def __init__(self, *args, **kwargs):
        ...
        if attrs is None:
            attrs = {}
        attrs.update({"data-provide": "datepicker", "data-date-format": "yyyy-mm-dd"})
        kwargs["attrs"] = attrs
