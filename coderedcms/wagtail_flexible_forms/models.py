...
from wagtail.contrib.forms.models import AbstractForm, AbstractEmailForm, AbstractFormSubmission
...
class Step:
    ...
    def get_form_class(self):
        return type("WagtailForm", self.steps.page.get_form_class_bases(), self.get_form_fields())
    ...
    @property
    def badge(self):
        return mark_safe('<span class="badge">%s/%s</span>') % (self.index1, len(self.steps))
    ...
    def get_existing_data(self, raw=False):
        ...
        if not raw:
            ...
            for field_name, value in data.items():
                if field_name in fields and isinstance(fields[field_name], FileField):
...
class Steps(list):
    def __init__(self, page, request=None):
        ...
        if has_steps:
            steps = [Step(self, i, form_field) for i, form_field in enumerate(self.form_fields)]
        else:
    ...
    def current(self, new_index: int):
        ...
        self.request.session[self.page.current_step_session_key] = self.clamp_index(new_index)
        ...
        if request.method == "POST":
            ...
            if step_value == "prev":
                ...
                return self.current.get_form_class()(
                    request.POST, request.FILES, initial=self.current.get_existing_data()
                )
        return self.current.get_form_class()(initial=self.current.get_existing_data())
    ...
    def save_files(self, form):
        ...
        for name, field in form.fields.items():
            if isinstance(field, FileField):
                ...
                path = storage.get_available_name(str(Path(directory) / file.name))
...
class SessionFormSubmission(AbstractFormSubmission):
    ...
    user = ForeignKey(
        settings.AUTH_USER_MODEL, null=True, blank=True, related_name="+", on_delete=PROTECT
    )
    ...
    def get_session(self):
        return import_module(settings.SESSION_ENGINE).SessionStore(session_key=self.session_key)
    ...
    def get_thumbnail_path(self, path, width=64, height=64):
        ...
        self.thumbnails_by_path = json.dumps(thumbnails_by_path, cls=StreamFormJSONEncoder)
        self.save()
        return thumbnail_path
    ...
    def get_files_by_field(self):
        ...
        for name, field in self.get_fields().items():
            if isinstance(field, FileField):
                path = data.get(name)
                if path:
                    files[name] = [path] + list(self.get_existing_thumbnails(path))
        return files
    ...
    def render_email(self, value):
        return mark_safe('<a href="mailto:%s" target="_blank">%s</a>') % (value, value)
    ...
    def format_db_field(self, field_name, raw=False):
        ...
        if raw:
            return value
        return self.format_value(self._meta.get_field(field_name).formfield(), value)
    ...
    def get_data(self, raw=False, add_metadata=True):
        ...
        if add_metadata:
            form_data.update(
                ...
                last_modification=self.format_db_field("last_modification", raw=raw),
            )
        return form_data
...
class SubmissionRevision(Model):
    ...
    CHANGED = "changed"
    DELETED = "deleted"
    TYPES = ((CREATED, _("Created")), (CHANGED, _("Changed")), (DELETED, _("Deleted")))
    ...
    def get_filters_for(submission):
        return {
            "submission_ct": ContentType.objects.get_for_model(submission._meta.model),
    ...
    def diff_summary(cls, page, data1, data2):
        ...
        for k, label in data_fields:
            ...
            if value2 == value1 or not value1 and not value2:
                continue
            is_hidden = isinstance(value1, hidden_types) or isinstance(value2, hidden_types)
            ...
            if value2 and not value1:
                ...
                diff.append(
                    (
                        (_("“%s” changed.") % label)
                        if is_hidden
                        else (_("“%s” changed from “%s” to “%s”.") % (label, value1, value2))
                    )
                )
        return "\n".join(diff)
    ...
    def create_from_submission(cls, submission, revision_type):
        ...
        try:
            previous = cls.objects.for_submission(submission).latest("created_at")
...
class StreamFormMixin:
    ...
    def get_context(self, request, *args, **kwargs):
        ...
        context.update(
            ...
            form=form,
            markups_and_bound_fields=list(self.steps.current.get_markups_and_bound_fields(form)),
        )
        return context
    ...
    def get_success_url(self):
        form_complete_models = [
            model for model in apps.get_models() if issubclass(model, FormCompleteMixin)
        ]
    ...
    def get_data_fields(self, by_step=False, add_metadata=True):
        if by_step:
            return [
                [(field_name, field.label) for field_name, field in step_fields.items()]
                for step_fields in self.get_form_fields(by_step=True)
            ]
...
class FormCompleteMixin:
    ...
    def serve(self, request, *args, **kwargs):
        ...
        if isinstance(form_page, LoginRequiredMixin) and not request.user.is_authenticated():
