...
class FormAdmin(ModelAdmin):
    ...
    menu_label = _("Forms")
    menu_icon = "form"
    list_display = ("title", "unprocessed_submissions_link", "all_submissions_link", "edit_link")
    ...
    def all_submissions_link(self, obj, label=_("See all submissions"), url_suffix=""):
...
class SubmissionStatusFilter(SimpleListFilter):
    ...
    unprocessed_status = ",".join((SessionFormSubmission.COMPLETE, SessionFormSubmission.REVIEWED))
...
class SubmissionURLHelper(AdminURLHelper):
    ...
    def _get_object_specific_action_url_pattern(self, action):
        return r"^%s/%s/%s/(?P<instance_pk>[-\w]+)/$" % (self.opts.app_label, "submissions", action)
...
class SubmissionButtonHelper(ButtonHelper):
    def set_status_button(
        ...
        url += "?status=" + status
        return {"url": url, "label": label, "classname": classnames, "title": title}
        ...
        if obj.status != obj.REVIEWED:
            status_buttons.append(
                self.reviewed_button(
                    pk, classnames_add=classnames_add, classnames_exclude=classnames_exclude
                )
            )
    ...
    def get(self, request, *args, **kwargs):
        ...
        if status in dict(self.model.STATUSES):
            ...
            if "revert" in request.GET:
                messages.success(request, "Reverted to the “%s” status." % verbose_label)
            else:
                revert_url = (
        ...
        if url is None:
            url = self.url_helper.get_action_url("index") + "?page_id=%s" % self.instance.page_id
        return redirect(url)
...
class SubmissionAdmin(ModelAdmin):
    ...
    def get_queryset(self, request):
        ...
        return qs.filter(page__in=form_pages).exclude(status=self.model.INCOMPLETE)
