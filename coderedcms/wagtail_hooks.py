...
def collapsible_js():
    return format_html(
        '<script src="{}"></script>', static("coderedcms/js/codered-editor.js")
    )
...
def codered_forms(user, editable_forms):
    ...
    """
    form_models = [
        model for model in get_page_models() if issubclass(model, CoderedFormMixin)
    ]
...
class CoderedFormAdmin(FormAdmin):
    ...
    def action_links(self, obj):
        ...
        if issubclass(type(obj.specific), CoderedFormPage):
            actions.append(
                '<a href="{0}">{1}</a>'.format(
                    reverse("wagtailforms:list_submissions", args=(obj.pk,)),
