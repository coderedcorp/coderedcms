...
* Added new ``wagtailcrx.fields.MonospaceField`` for rendering text area fields
...
* **AMP support has been removed**. This means that your site will no longer
  ...
  * ``wagtailcrx_tags.convert_to_amp`` has been removed.

  * ``wagtailcrx.utils.convert_to_amp()`` has been removed.

  * You should remove any ``amp_template`` attributes from your models, as they
    are no longer used.

* ``wagtailcrx/pages/base.html`` template has been modified. If you implement or
...
* Properties and templates associated with SEO functionality have changed:
  ...
  * ``wagtailcrx_tags.og_image`` -> ``page.seo_image_url``

  * ``wagtailcrx_tags.structured_data_datetime`` has been removed.

* Some built-in blocks used in the Wagtail admin have been replaced:

  * ``wagtailcrx.blocks.MultiSelectBlock`` -> ``wagtail.core.blocks.MultipleChoiceBlock``

  * ``wagtailcrx.blocks.OpenHoursValue`` -> ``wagtailseo.blocks.OpenHoursValue``

  * ``wagtailcrx.blocks.OpenHoursBlock`` -> ``wagtailseo.blocks.OpenHoursBlock``

  * ``wagtailcrx.blocks.StructuredDataActionBlock`` -> ``wagtailseo.blocks.StructuredDataActionBlock``
...
* You may need to find/replace
  ``wagtailcrx.blocks.base_blocks.MultiSelectBlock`` with
...
Thanks to everyone who contributed to `0.21.0 on GitHub <https://github.com/coderedcorp/wagtailcrx/milestone/31?closed=1>`_.
